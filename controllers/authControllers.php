...
if(($dob>2018)){
	$errors['dob']="Select your correct age";
}

$emailQuery="SELECT * FROM register_tb WHERE username=? limit 1";
...
if(count($errors)===0){
	...
	mysqli_query($conn,"insert into register_tb(fname,mname,lname,dob,home_town,school,level_education,picture,password,username)VALUES('$fname','$mname','$lname','$dob','$hometown','$school','$level_edu','$image','$password','$username')");
   // if(mysqli_query($conn,$sql)){
          header('location: index.php');
      }
      else{ echo '<script>
     alert("Your image  should 1mb or less");
...
$sql="SELECT * FROM register_tb where username='$username' and password='$password' limit 1";
...
$update="update `register_tb` set `password`='$password' where `username`='$username'";
